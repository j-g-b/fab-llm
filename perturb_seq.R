...
#
pdf(file = "figures/perturb.pdf", family = "Times", height = 5, width = 5)
fdr_rp <- p.adjust(p_rp, method = "BH")
fdr_fab <- p.adjust(p_fab, method = "BH")
fdr_fab_splt <- p.adjust(p_fab_splt, method = "BH")
plot(1:length(fdr_rp), fdr_rp[order(fdr_rp)], type = 'l', lwd = 2,
     ylim = c(0, 1), xlab = "Rank", ylab = "FDR")
lines(1:length(fdr_fab), fdr_fab[order(fdr_fab)], type = 'l', lwd = 2, col = "#08519c")
lines(1:length(fdr_fab_splt), fdr_fab_splt[order(fdr_fab_splt)], type = 'l', lwd = 2, col = "#756bb1")
abline(h = 0.1, col = "#a50f15")
legend(x = 6000, y = 0.6, legend = c("RP test", "AFAB test", "AFAB SS test"), col = c("black", "#08519c", "#756bb1"), lwd = c(2, 2, 2))
dev.off()
...
saveRDS(sens_res, file = "results/perturb_seq100_sens.rds")
